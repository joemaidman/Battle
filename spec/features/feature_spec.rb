...
feature 'A new game' do
  ...
  feature "Players want to start a game", :type => :feature do
    describe "so enter names on the '/' route and are sent to /play route which" do
      it "shows players names" do
        expect(page).to have_text("#{p1_name}")
        expect(page).to have_text("#{p2_name}")
      end
    end
  end
  ...
  feature 'winning & losing' do
    describe "when a player reaches 0 HP" do
      ...
      it "it tells the losing player they have lost and the winning player they won" do
        expect(page).to have_text("#{p2_name} won!! Sorry #{p1_name} you lost!!")
      end
    end
  end

  feature "Show player health points", :type => :feature do
    describe "Players are shown their HP on the play page" do
      before do
        sign_in_and_play(p1_name, p2_name)
      end
      it "shows player one's HP" do
        expect(page.find_by_id("p1_hp")).to have_content("HP: 100")
      end
      it "shows player two's HP" do
        expect(page.find_by_id("p2_hp")).to have_content("HP: 100")
      end
    end
  end
  ...
  feature "Attacking", :type => :feature do
    describe "player 1 attacks player 2" do
      ...
      it "and the attacked player's HP is reduced" do
        ...
        click_button("Next Turn")
        expect(page.find_by_id("p1_hp")).to have_content("HP: 94")
        expect(page.find_by_id("p2_hp")).to have_content("HP: 100")
      end
